...
export const GET: APIRoute = async ({ request, locals }: any) => {
  ...
  const [orders, products, payments, sweeps] = await Promise.all([
    ...
    env.DB.prepare("SELECT COUNT(*) AS count, COALESCE(SUM(CAST(amount AS INTEGER)), 0) AS amount FROM sweep_tasks WHERE status IN ('pending','submitted')").first(),
  ]);
  return apiOk({ orders: orders.results || [], products, payments: payments.results || [], sweeps });
};
